...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/generate-story-stream", async (req, res) => {
    ...
    try {
      ...
      console.error("Error streaming story:", error instanceof Error ? error.message : error);
      if (res.headersSent) {
...
Style: ${artStyle}. Close-up friendly portrait, expressive eyes, child-safe content, suitable for ages 3-9. No scary elements, no weapons. Circular portrait composition with a cosmic/starry background.`;
      ...
      console.error("Error generating avatar:", error instanceof Error ? error.message : error);
...
Style: ${sceneStyle}. Wide landscape composition, magical atmosphere, child-safe content, suitable for ages 3-9. No scary elements. Warm, cozy, wonder-filled.`;
      ...
      console.error("Error generating scene:", error instanceof Error ? error.message : error);
  ...
  app.post("/api/suggest-settings", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        return res.status(500).json({ error: "Failed to parse suggestion response" });
      }
      ...
      console.error("Suggest settings error:", error instanceof Error ? error.message : error);
  ...
  app.post("/api/tts-preview", async (req, res) => {
    ...
    try {
      ...
      console.error("TTS preview error:", error instanceof Error ? error.message : error);
